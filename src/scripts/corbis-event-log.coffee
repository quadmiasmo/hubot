...
module.exports = (robot) ->
    ...
    #
    # responds to:
    #   <robotName> [corbis ]error[ me] <guid>[ in <environment>][ with stacktrace]
    #
    robot.respond /(corbis )?error(?: me| show)? "?([0-9a-f]{8}-(?:[0-9a-f]{4}-){3}[0-9a-f]{12})"?(?: (?:in|for|from|use) ([a-z]+)\b)?( (?:include |with )?stack ?trace)?/i, (msg) ->
    ...
    #
    robot.respond /(corbis )?errors(?: me| show)?(?: (?:top )?(\d+))?(?: (?:in|for|from|use) ([a-z]+)\b)?/i, (msg) ->
        validateRequest msg, (errorlog) ->
...
class ErrorLog
    constructor: (msg) ->
        ...
        @includeStack   = msg.match[4] isnt undefined
    ...
    format: () ->
        ...
        message = "MESSAGE: \"#{@Message}\" | LOGGED AT: #{@LoggedAt} | MACHINE: #{@HostName} | ACTIVITY UID: #{@ActivityUid}"

        return if @includeStack then "#{message}\nSTACK TRACE:\n#{@StackTrace}" else message
